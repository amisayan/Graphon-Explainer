...
def align_x_graphs(graphs: List[np.ndarray], node_x: List[np.ndarray], padding: bool = False, N: int = None) -> Tuple[List[np.ndarray], List[np.ndarray], int, int]:
    ...
    for i in range(len(graphs)):
        ...
        node_degree = 0.5 * np.sum(graphs[i], axis=0) + 0.5 * np.sum(graphs[i], axis=1)
        node_degree /= np.sum(node_degree)
        print("Node degree is", node_degree, len(node_degree))
        idx = np.argsort(node_degree)  # ascending
        #print("Idx is",idx)
        idx = idx[::-1]  # descending
        print("idx is",idx)
        ...
        node_x = copy.deepcopy( node_x[i] )
        
        print("nodex is",node_x)
        print("length of nodex is",len(node_x))
        ...
        if padding:
            ...
            # added
            aligned_node_x = np.zeros((max_num, 7))
...
def align_x_graphscorrected(graphs: List[np.ndarray], node_x: List[np.ndarray], padding: bool = False, N: int = None) -> Tuple[List[np.ndarray], List[np.ndarray], int, int]:
    """
    Align multiple graphs by sorting their nodes by descending node degrees
    ...
    for i in range(len(graphs)):
        ...
        x_node = copy.deepcopy( node_x[i] )
        
        print("nodex is",x_node)
        print("length of nodex is",len(x_node))
        #sorted_node_x = node_x[ idx, :]
        sorted_node_x=x_node[idx]
...
def two_graphons_mixup(two_graphons, la=0.5, num_sample=20, show=False):
    ...
    if show:
        plt.axis('off')
        plt.imshow(new_graphon,cmap='inferno')



    sample_graph_label = torch.from_numpy(label).type(torch.float32)
    
    #print("sample graph label is",sample_graph_label)
    # print(new_graphon)
    ...
    for i in range(num_sample):
        ...
        pyg_graph = Data()
        #print("Dhokanor agey",sample_graph_label)
        pyg_graph.y = sample_graph_label
        #print("Graph.y is",pyg_graph.y)
        pyg_graph.edge_index = edge_index
        #print("Edge index is",pyg_graph.edge_index.size())
        pyg_graph.num_nodes = num_nodes
        sample_graphs.append(pyg_graph)
        #print("Obtained Graph is",pyg_graph,pyg_graph.y)
        
        
        # print(edge_index)
    return sample_graphs

def four_graphons_mixup(four_graphons, num_sample=20,la1=0.25,la2=0.25,la3=0.25,la4=0.25, show=False):
    

    label = la1 * four_graphons[0][0] + la2 * four_graphons[1][0]+la3*four_graphons[2][0]+la4*four_graphons[3][0]
    #print("label is",label)
    new_graphon = la1 * four_graphons[0][1] + la2 * four_graphons[1][1]+ la3*four_graphons[2][1]+la4*four_graphons[3][1]
    if show:
        plt.axis('off')
        plt.imshow(new_graphon,cmap='inferno')
...
def split_class_x_graphs(dataset):
    ...
    for graph in dataset:
        ...
        all_graphs_list.append(adj)
        x=graph.x.numpy()
        all_node_x_list.append(x)

    #print(all_node_x_list)
    #print("Length of node list",len(all_node_x_list))
    class_graphs = []
    for class_label in set(y_list):
        c_graph_list = [all_graphs_list[i] for i in range(len(y_list)) if y_list[i] == class_label]
